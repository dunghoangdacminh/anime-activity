export async function fetchAnimeData() {
  ...
  export async function searchAnime(keyword: string) {
	try {
	  ...
	  const response = await fetch(`/api/proxy?url=https://akimiya-api-url.vercel.app/api/search?keyword=${encodeURIComponent(keyword)}`)
  ...
  export async function fetchAnimeDetails(id: string) {
	try {
	  ...
	  const response = await fetch(`/api/proxy?url=https://akimiya-api-url.vercel.app/api/info?id=${encodeURIComponent(id)}`)
